...
class DataParameter():
    def __init__(self, model_kwargs, args) -> None:
        self.name = model_kwargs['name'][0].split('-')[0]
        self.image = (Denormalize(model_kwargs['low_res'][0].cpu().numpy()) * 255).astype(np.uint8).transpose(1, 2, 0)
        
        # Create image crops without density maps
        create_overlapping_crops_without_density(model_kwargs, args)
        ...
        # Result parameters
        self.mae = np.full(model_kwargs['low_res'].size(0), np.Inf)
        self.result = np.zeros(model_kwargs['low_res'].size()) 
        self.result = np.mean(self.result, axis=1, keepdims=True)       
...
#     # Ensure args.normalizer is a tensor and reshape it appropriately
#     normalizer = th.tensor(args.normalizer, dtype=model_kwargs['high_res'].dtype).view(1, -1, 1, 1).to(model_kwargs['high_res'].device)
#     model_kwargs['crowd_count'] = th.sum((model_kwargs['high_res'] + 1) * 0.5 * normalizer, dim=(1, 2, 3)).cpu().numpy()
...
def create_overlapping_crops_without_density(model_kwargs, args):
    image = model_kwargs['low_res']
    model_kwargs['dims'] = image.shape[-2:]
