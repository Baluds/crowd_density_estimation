...
# from your_module import DataParameter
...
def main():
    ...
    for _ in os.listdir(args.data_dir):
        ...
        # Check if batch size is zero
        if model_kwargs['low_res'].size(0) == 0:
            continue  # Skip processing if batch size is zero
        ...
        while data_parameter.resample:
            ...
            path_write=os.path.join(args.log_dir, f'{data_parameter.name}.jpg'), req_image[:, :, ::-1]
            print(f"path is {path_write}")
            cv2.imwrite(path_write)
    print(args.log_dir)
...
def organize_crops(model_kwargs):
    ...
    if len(indices[0]) == 0:
        # No samples with crowd_count > 0
        model_kwargs['low_res'] = model_kwargs['low_res'][indices]
        model_kwargs['empty'] = True
    else:
        model_kwargs['order'] = model_kwargs['order'][indices]
        model_kwargs['low_res'] = model_kwargs['low_res'][indices]
        model_kwargs['empty'] = False
...
def load_data_for_worker(base_samples, batch_size, normalizer, pred_channels, file_name, class_cond=False):
    ...
    while True:
        ...
        # Yield any remaining data if buffer is not empty
        if len(buffer) > 0:
            batch = th.from_numpy(np.stack(buffer)).float()
            batch = batch / 127.5 - 1.0
            batch = batch.permute(0, 3, 1, 2)
